...
if (is_on($config['plugins'][$name]))
{
    ...
    $plugin[$name]['non_auth_actions'][] = 'afLogin';

    //...... Set up non authenticated pages (preg_match) (if applicable)
    $plugin[$name]['non_auth_pages'][]	= 'afLogin';
    $plugin[$name]['non_auth_pages'][]	= 'aflost_password';
	...
	/*********************************************************/
	/* Should only ever check this if logged in as admin */
